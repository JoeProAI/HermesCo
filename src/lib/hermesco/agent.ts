...
function systemPrompt(treasurySnapshot: string): string {
  ...
  return [
    ...
    "   credits the revenue, books the compute spend through the Treasury (NemoClaw screens it),",
    "   provisions your Fly machine, runs the real job, and returns the deliverable.",
    "   Profit = price minus compute cost. If the compute spend needs human approval, say so,",
    "   then call deliver_job again once it is approved.",
    "",
    "PRINCIPLES:",
